...
logger = logging.getLogger('SSDNet.Train')
...
parser.add_argument('--search-hyperparameter', default=None, help='Whether to search hyperparameter, BO, GS') #Bayesian Optimization, or Grid Search
...
# parser.add_argument('--model-name', default='base_model_exchange_loss0', help='Directory containing params.json')
...
# parser.add_argument('--model-name', default='base_model_Sanyo_loss0', help='Directory containing params.json')
# parser.add_argument('--model-name', default='base_model_Hanergy', help='Directory containing params.json')
# parser.add_argument('--model-name', default='base_model_Hanergy_loss0', help='Directory containing params.json')
...
parser.add_argument('--restore-file', default='best',#restore the saved model,
...
def main():
    ...
    if args.search_hyperparameter is not None:
        ...
        para_base_model_json_path = os.path.join(os.path.join('experiments', args.model_name), 'params.json')
    ...
    if cuda_exist:
        ...
        model = Models_Sanyo.SSDNet(params,args.dataset).cuda()
        torch.backends.cudnn.enabled = True
        torch.backends.cudnn.benchmark = True
    else:
        params.device = torch.device('cpu')
        logger.info('Not using cuda...')
        model = Models_Sanyo.SSDNet(params,args.dataset)
